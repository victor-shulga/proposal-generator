...
description: >
  ...
  (quote-vs-proposal reframe). Reusable across ALL clients — first reference
  build was an MEP engineering client. Trigger when Viktor says: "зроби пропоузал для [client]",
...
proposal reaches the buyer**. Reusable for every client. Gold-standard reference
build: an MEP engineering client — kept in the private workspace, live at
https://client-a-proposals.netlify.app (call-deck.html + send-proposal.html + index.html).
