...
description: >
  ...
  (quote-vs-proposal reframe). Reusable across ALL clients. Trigger when Viktor says: "зроби пропоузал для [client]",
...
proposal reaches the buyer**. Reusable for every client. A gold-standard reference
build (call-deck.html + send-proposal.html + index.html) lives in the local private
